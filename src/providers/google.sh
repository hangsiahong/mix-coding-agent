...
# On Vertex/Gemini, if we mess with tool calls and turn them into text strings 
# like "[Used tools: ...]", the model will see this in the history and start 
# hallucinating text-based tool calls instead of using the JSON API!
# We just pass the history as-is.
