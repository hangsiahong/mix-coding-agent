...
while true; do
  if [ "$INTERACTIVE" = true ]; then
    ...
    # Shorten: keep first letter of each dir except last
    _prompt_short=""
    _IFS_save="$IFS"; IFS='/'
    _parts=($_prompt_cwd)
    IFS="$_IFS_save"
    _plen=${#_parts[@]}
    for (( _pi=0; _pi<_plen-1; _pi++ )); do
      _d="${_parts[$_pi]}"
      [ -n "$_d" ] && _prompt_short+="/${_d:0:1}"
    done
    [ "$_plen" -gt 0 ] && _prompt_short+="/${_parts[$((_plen-1))]}"
