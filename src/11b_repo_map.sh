...
_REPO_MAP_PATTERNS=(
  ...
  "js:^(export |function |class |const |let |interface |type ):decl"
  "ts:^(export |function |class |const |let |interface |type |enum ):decl"
  "go:^(func |type |var |const |import ):decl"
  "rs:^(pub )?(fn |struct |enum |trait |impl |mod ):decl"
  "java:^(public |private |protected |class |interface |enum |void ):decl"
